...
for video in video_list:
	...
	t_loss = []
	for st in range(0, len(out)):
		#t_loss.append(np.mean((out[st, 3:] - gt[st, 3:6])**2))
		t_loss.append(((out[st, 3:] - gt[st, 3:6])))
		
	plt.plot(t_loss)
	plt.savefig('absPose.png')

	rel_gt = []
	rel_pred = []
	rel_gt.append(gt[0, 3:6])
	rel_pred.append(out[0, 3:])
	for st in range(1, len(out)):
		rel_gt.append(gt[st-1, 3:6]-gt[st, 3:6])
		rel_pred.append(out[st-1, 3:]-out[st, 3:])

	
	t_loss = []
	for st in range(0, len(rel_gt)):
		#t_loss.append(np.mean((rel_gt[st] - rel_pred[st])**2))
		t_loss.append(((rel_gt[st] - rel_pred[st])))

	plt.figure()
	plt.plot(t_loss)
	plt.savefig('relPose.png')
